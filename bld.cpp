...
void add_flags(bld::Command &cmd) { cmd.add_parts("-Wall", "-Wextra", "-Wno-missing-field-initializers"); }

void add_include_flags(bld::Command &cmd) { cmd.add_parts("-I./"); }

void add_linker_flags(bld::Command &cmd)
{
    cmd.add_parts("./raylib/lib/libraylib.a", "-L./ " "-lm");
}
...
void add_build_source(bld::Command &cmd)
{
    ...
    for (auto &file : files)
    {
        if (bld::fs::get_extension(file) == ".cpp")
        {
            cmd.add_parts(file);
        }
    }
