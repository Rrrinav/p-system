...
public:
            ...
            _workers.emplace_back([this]
            {
                while (true)
                {
                    std::function<void()> task;
                    {
                        ...
                        if (_stop && _task_queue.empty()) return; // Exit thread safely
    ...
    void wait()
    {
        ...
        _completion_cv.wait(lock, [this] { return _active_tasks.load(std::memory_order_acquire) == 0; });
    }
};
